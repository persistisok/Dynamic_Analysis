    ...
    if (maps_file != NULL) {
        ...
        while (fgets(line, sizeof(line), maps_file)) {
            if (strstr(line, "libc.so") != NULL) {
                ...
                unsigned long dlo_opc = ptrace(PTRACE_PEEKTEXT,child,dlopen_addr,0);
                printf("dlopen_opc: %lx\n",dlo_opc);
                dlo_opc = ptrace(PTRACE_PEEKTEXT,child,dlopen_addr+8,0);
                printf("dlopen_opc: %lx\n",dlo_opc);
                dlo_opc = ptrace(PTRACE_PEEKTEXT,child,dlopen_addr+16,0);
                printf("dlopen_opc: %lx\n",dlo_opc);
                break;
                }
            }
