    ...
    if ((bfd_get_file_flags(abfd) & HAS_SYMS) != 0)
    {
        ...
        for (int i = 0; i < num_symbols; i++)
        {
            ...
            if (strcmp(symbol_name, function_name) == 0)
            {
                ...
                asection *function_section = bfd_get_section_by_name(abfd,symbol->name);

                //Iterate over the instructions to find function calls
                disassemble_info disasm_info;
                init_disassemble_info(&disasm_info, stdout, (fprintf_ftype)fprintf);
                disasm_info.flavour = bfd_target_elf_flavour;
                disasm_info.arch = bfd_get_arch(abfd);
                disasm_info.mach = bfd_get_mach(abfd);
                disasm_info.read_memory_func = (int (*)(bfd_vma, bfd_byte *, unsigned int, struct disassemble_info *))bfd_get_section_contents;
