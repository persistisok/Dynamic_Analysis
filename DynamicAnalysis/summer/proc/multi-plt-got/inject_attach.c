    ...
    if (maps_file != NULL) {
        ...
        while (fgets(line, sizeof(line), maps_file)) {
            if (strstr(line, "libc.so.6") != NULL) {
                ...
                printf("Addr of libc's start:%lx\n",start);
...
void inject_lib(pid_t child, unsigned long lib_addr, unsigned long dlopen_addr){
    ...
    unsigned long lib_path = ptrace(PTRACE_PEEKTEXT,child,lib_addr,0);
    printf("lib_path: %lx\n",lib_path);
    ...
    ptrace(PTRACE_GETREGS,child,NULL,&r2);
        ...
        struct user_regs_struct r3;
        ptrace(PTRACE_GETREGS,child,NULL,&r3);
        printf("rip:%llx rdi: %llx rsi: %llx\n",r3.rip,r3.rdi,r3.rsi);
        // call r9; int 3
        ...
        ptrace(PTRACE_GETREGS,child,NULL,&r3);
        printf("rip: %llx rax: %llx rdi: %llx rsi: %llx\n",r3.rip,r3.rdi,r3.rsi);
...
int main(int argc, const char* argv[]) {
    ...
    printf("func_origin's got_addr: %lx\n",got_addr);
