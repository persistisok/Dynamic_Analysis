    ...
    if (maps_file != NULL) {
        ...
        while (fgets(line, sizeof(line), maps_file)) {
            if (strstr(line, lib_name) != NULL) {
                ...
                sscanf(line, "%lx-%lx", &start, &end);
                // printf("Addr of ./libc.so start:%lx\n", start);
                unsigned long offset = 0x1147;  // 静态文件中的地址偏移量
...
int main(int argc, char *argv[]) {
    ...
    if (child == 0) {
        ...
        unsigned char* path = "./libInject.so";
        unsigned long lib_addr = put_lib_path(child,path);

        unsigned long dlopen_addr = get_dlopen_addr(child);

        inject_lib(child,lib_addr,dlopen_addr);

        unsigned char* lib_name = "libInject.so";
        unsigned char* func_name_inject = "print_sentence_inject";
        unsigned long func_addr = get_func_addr(child,lib_name,func_name_inject);

        unsigned char* prog_name = "circle";
        unsigned char* func_name_origin = "print_sentence";
